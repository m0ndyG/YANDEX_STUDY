...
def handle_start(message):
    bot.send_message(message.chat.id,"приветствую я преобразующий в текст голосовые сообщения бот пример моих возможностей в следуюшем собшении чтобы использовать мои возможности /stt если есть вопросы /help")
    with open('tts.ogg', 'rb') as voice:
        bot.send_voice(message.chat.id, voice.read())

@bot.message_handler(commands=['help'])
def handle_help(message):
    bot.send_message(message.chat.id,"""Приветствую вас я телеграмм бот для преобразование в текст я обладаю лимитами 200 символов на сообшение и 1000 на пользователя чтобы начать преобразования /stt""")
